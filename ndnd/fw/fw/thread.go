...
func (t *Thread) processIncomingInterest(packet *defn.Pkt) {
	...
	if len(allowedNetNexthops) > 0 {
		// BFIR: when multiple egress routers are known for this prefix and BIER is
		// enabled, encode a BIER bit-string so BierStrategy can replicate the
		// Interest statelessly via the BIFT.
		//
		// Ideally this condition would also check PetEntry.Multicast to limit BIER
		// to Sync group prefixes (as opposed to multihomed producer prefixes).
		// The Multicast flag is propagated end-to-end from DV through the PET;
		// setting it requires the SVS library to mark group prefixes at announce
		// time, which is left as a future integration step.
		if IsBierEnabled() && len(allowedNetER) > 1 && len(packet.Bier) == 0 {
		...
		// Auto-select BierStrategy when packet has BIER header (BFIR or transit BFR)
		activeStrategy := strategy
		if len(packet.Bier) > 0 {
			if bs, ok := t.strategies[BierStrategyName.Hash()]; ok {
				activeStrategy = bs
			}
		}
		// Pass to strategy AfterReceiveInterest pipeline
		activeStrategy.AfterReceiveInterest(packet, pitEntry, incomingFace.FaceID(), allowedNetNexthops, allowedNetER)
	} else {
		// NACK?
	}
}
