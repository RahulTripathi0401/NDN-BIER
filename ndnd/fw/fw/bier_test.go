...
func TestBierBitManipulation(t *testing.T) {
	...
	t.Run("BierAnd", func(t *testing.T) {
	...
	t.Run("BierIsZero", func(t *testing.T) {
		zero := []byte{0, 0, 0}
		nonZero := []byte{0, 0, 1}
		...
		if BierIsZero(nonZero) {
			t.Error("Non-zero bitstring should return false")
		}
	})

	t.Run("BierClone", func(t *testing.T) {
		original := []byte{1, 2, 3, 4}
		cloned := BierClone(original)
...
func TestBiftConstruction(t *testing.T) {
	...
	t.Run("UpdateNextHop and RebuildFbm", func(t *testing.T) {
		...
		if face200Fbm == nil {
			...
			if !BierGetBit(face200Fbm, 5) {
				t.Error("Face 200 F-BM should have bit 5 set")
			}
			if BierGetBit(face200Fbm, 0) {
...
func TestBierReplicationMask(t *testing.T) {
	...
	fbm1 = BierSetBit(fbm1, 0)
	fbm1 = BierSetBit(fbm1, 1)
	fbm1 = BierSetBit(fbm1, 2)

	fbm2 = BierSetBit(fbm2, 3)
	fbm2 = BierSetBit(fbm2, 4)

	fbm3 = BierSetBit(fbm3, 5)
	fbm3 = BierSetBit(fbm3, 6)

	// Incoming BIER bit-string has bits 0, 3, 5 set (3 egress routers)
	var incoming []byte
	incoming = BierSetBit(incoming, 0)
	incoming = BierSetBit(incoming, 3)
	incoming = BierSetBit(incoming, 5)

	// Compute replication masks
	rep1 := BierAnd(incoming, fbm1)
	rep2 := BierAnd(incoming, fbm2)
	rep3 := BierAnd(incoming, fbm3)

	// Neighbor 1 should replicate for bit 0 only
	if !BierGetBit(rep1, 0) {
		t.Error("Neighbor 1 should replicate for bit 0")
	}
	if BierGetBit(rep1, 1) || BierGetBit(rep1, 2) {
		t.Error("Neighbor 1 should not replicate for bits 1 or 2")
	}

	// Neighbor 2 should replicate for bit 3 only
	if !BierGetBit(rep2, 3) {
		t.Error("Neighbor 2 should replicate for bit 3")
	}
	if BierGetBit(rep2, 4) {
		t.Error("Neighbor 2 should not replicate for bit 4")
	}

	// Neighbor 3 should replicate for bit 5 only
	if !BierGetBit(rep3, 5) {
		t.Error("Neighbor 3 should replicate for bit 5")
	}
	if BierGetBit(rep3, 6) {
