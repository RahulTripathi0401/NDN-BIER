...
BOOST_AUTO_TEST_CASE(SendBierBitStringInterest)
{
  const std::vector<uint8_t> expectedMask = {0x80, 0x01};
  auto interest = makeInterest("/bier/test");
  interest->setTag(make_shared<lp::BierBitStringTag>(expectedMask));

  face->sendInterest(*interest);

  BOOST_REQUIRE_EQUAL(transport->sentPackets.size(), 1);
  lp::Packet sent(transport->sentPackets.back());
  BOOST_REQUIRE(sent.has<lp::BierBitStringField>());
  const auto& mask = sent.get<lp::BierBitStringField>();
  BOOST_CHECK_EQUAL_COLLECTIONS(mask.begin(), mask.end(), expectedMask.begin(), expectedMask.end());
}

BOOST_AUTO_TEST_CASE(ReceiveBierBitStringInterest)
{
  const std::vector<uint8_t> expectedMask = {0x40, 0x02};
  auto interest = makeInterest("/bier/test");
  lp::Packet packet(interest->wireEncode());
  packet.set<lp::BierBitStringField>(expectedMask);

  transport->receivePacket(packet.wireEncode());

  BOOST_REQUIRE_EQUAL(receivedInterests.size(), 1);
  auto tag = receivedInterests.back().getTag<lp::BierBitStringTag>();
  BOOST_REQUIRE(tag != nullptr);
  const auto& receivedMask = tag->get();
  BOOST_CHECK_EQUAL_COLLECTIONS(receivedMask.begin(), receivedMask.end(),
                                expectedMask.begin(), expectedMask.end());
}
