...
BOOST_AUTO_TEST_CASE(BierReplicationPrePit)
{
  auto ingress = addFace();
  auto egress1 = addFace();
  auto egress2 = addFace();

  forwarder.m_config.bierBitFaceMap.clear();
  forwarder.m_config.bierPrefixMap.clear();
  forwarder.m_config.bierBitFaceMap.emplace(0, egress1->getId());
  forwarder.m_config.bierBitFaceMap.emplace(9, egress2->getId());

  auto interest = makeInterest("/bier/test");
  interest->setTag(make_shared<lp::BierBitStringTag>(std::vector<uint8_t>{0x80, 0x40}));
  ingress->receiveInterest(*interest);
  this->advanceClocks(1_ms, 5_ms);

  BOOST_REQUIRE_EQUAL(egress1->sentInterests.size(), 1);
  BOOST_REQUIRE_EQUAL(egress2->sentInterests.size(), 1);
  BOOST_CHECK_EQUAL(forwarder.getPit().size(), 0);
  BOOST_CHECK_EQUAL(counters.nInInterests, 1);
  BOOST_CHECK_EQUAL(counters.nOutInterests, 2);
}

BOOST_AUTO_TEST_CASE(BierLocalFallbackToNdnPipeline)
{
  auto ingress = addFace();
  auto egress = addFace();

  Fib& fib = forwarder.getFib();
  fib::Entry* entry = fib.insert("/bier").first;
  fib.addOrUpdateNextHop(*entry, *egress, 0);

  forwarder.m_config.bierBitFaceMap.clear();
  forwarder.m_config.bierPrefixMap.clear();
  forwarder.m_config.bierLocalBit = 0;
  forwarder.m_config.bierPrefixMap.emplace(Name("/bier"), std::vector<uint8_t>{0x80});

  auto interest = makeInterest("/bier/demo");
  ingress->receiveInterest(*interest);
  this->advanceClocks(1_ms, 5_ms);

  BOOST_REQUIRE_EQUAL(egress->sentInterests.size(), 1);
  BOOST_CHECK(egress->sentInterests[0].getTag<lp::BierBitStringTag>() == nullptr);
  BOOST_CHECK_EQUAL(counters.nInInterests, 1);
  BOOST_CHECK_EQUAL(counters.nOutInterests, 1);
}

BOOST_AUTO_TEST_CASE(BierRuntimeControlInterests)
{
  auto face1 = addFace("dummy://", "dummy://", ndn::nfd::FACE_SCOPE_LOCAL);

  auto clear = makeInterest("/localhost/nfd/bier/clear");
  forwarder.onIncomingInterest(*clear, FaceEndpoint(*face1));

  auto localBit = makeInterest("/localhost/nfd/bier/local-bit/4");
  forwarder.onIncomingInterest(*localBit, FaceEndpoint(*face1));
  BOOST_TEST_REQUIRE(forwarder.m_config.bierLocalBit.has_value());
  BOOST_TEST(*forwarder.m_config.bierLocalBit == 4);

  auto bitFace = makeInterest("/localhost/nfd/bier/bit-face/2/555");
  forwarder.onIncomingInterest(*bitFace, FaceEndpoint(*face1));
  BOOST_TEST_REQUIRE(forwarder.m_config.bierBitFaceMap.count(2) == 1);
  BOOST_TEST(forwarder.m_config.bierBitFaceMap.at(2) == 555);

  auto prefix = makeInterest("/localhost/nfd/bier/prefix/8040/bier/demo");
  forwarder.onIncomingInterest(*prefix, FaceEndpoint(*face1));
  BOOST_TEST_REQUIRE(forwarder.m_config.bierPrefixMap.count(Name("/bier/demo")) == 1);
  const auto& mask = forwarder.m_config.bierPrefixMap.at(Name("/bier/demo"));
  BOOST_TEST(mask.size() == 2);
  BOOST_TEST(mask[0] == 0x80);
  BOOST_TEST(mask[1] == 0x40);
  BOOST_TEST(forwarder.getPit().size() == 0);
}
...
BOOST_AUTO_TEST_CASE(BierConfig)
{
  ConfigFile cf;
  forwarder.setConfigFile(cf);
  ...
  cf.parse(config, false, "dummy-config");
  BOOST_TEST_REQUIRE(forwarder.m_config.bierLocalBit.has_value());
  BOOST_TEST(*forwarder.m_config.bierLocalBit == 7);
  BOOST_TEST_REQUIRE(forwarder.m_config.bierPrefixMap.count(Name("/bier/demo")) == 1);
  const auto& mask = forwarder.m_config.bierPrefixMap.at(Name("/bier/demo"));
  BOOST_TEST(mask.size() == 2);
  BOOST_TEST(mask[0] == 0x80);
  BOOST_TEST(mask[1] == 0x40);
  BOOST_TEST_REQUIRE(forwarder.m_config.bierBitFaceMap.count(0) == 1);
  BOOST_TEST_REQUIRE(forwarder.m_config.bierBitFaceMap.count(9) == 1);
  BOOST_TEST(forwarder.m_config.bierBitFaceMap.at(0) == 101);
  BOOST_TEST(forwarder.m_config.bierBitFaceMap.at(9) == 202);
}
