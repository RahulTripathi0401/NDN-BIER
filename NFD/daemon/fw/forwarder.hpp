...
private:
  bool
  processBierInterest(const Interest& interest, const FaceEndpoint& ingress);

  bool
  processBierControlInterest(const Interest& interest, const FaceEndpoint& ingress);

  std::vector<uint8_t>
  findBierPrefixMask(const Name& name) const;
...
NFD_PUBLIC_WITH_TESTS_ELSE_PRIVATE:
  ...
  struct Config
  {
    ...
    /// Prefix-to-BIER bitstring map used by BFIR for initial mask construction.
    std::map<Name, std::vector<uint8_t>> bierPrefixMap;

    /// Mapping from BIER bit position to outgoing face id.
    std::unordered_map<uint32_t, uint64_t> bierBitFaceMap;

    /// Local BIER bit position of this router (optional).
    std::optional<uint32_t> bierLocalBit;
  };
  Config m_config;
