...
static std::string
normalizeHexString(std::string value)
{
...
decodeHexMask(std::string value)
{
  value = normalizeHexString(std::move(value));
  if (value.empty()) {
    return {};
  }
  ...
  for (size_t i = 0; i < value.size(); i += 2) {
    auto decodeNibble = [] (char c) -> uint8_t {
      ...
      NDN_THROW(ConfigFile::Error("Invalid hexadecimal BIER mask"));
    };

    uint8_t msn = decodeNibble(value[i]);
    uint8_t lsn = decodeNibble(value[i + 1]);
    mask.push_back(static_cast<uint8_t>((msn << 4) | lsn));
  }
  return mask;
}

static bool
isBitSet(const std::vector<uint8_t>& mask, uint32_t bitPos)
{
  size_t byteIndex = bitPos / 8;
  if (byteIndex >= mask.size()) {
    return false;
  }
  uint8_t bit = static_cast<uint8_t>(7 - (bitPos % 8));
  return (mask[byteIndex] & (1U << bit)) != 0;
}

static void
ensureBitCapacity(std::vector<uint8_t>& mask, uint32_t bitPos)
{
  size_t byteIndex = bitPos / 8;
  if (byteIndex >= mask.size()) {
    mask.resize(byteIndex + 1, 0);
  }
}

static void
setBit(std::vector<uint8_t>& mask, uint32_t bitPos)
{
  ensureBitCapacity(mask, bitPos);
  uint8_t bit = static_cast<uint8_t>(7 - (bitPos % 8));
  mask[bitPos / 8] = static_cast<uint8_t>(mask[bitPos / 8] | (1U << bit));
}

static void
clearBit(std::vector<uint8_t>& mask, uint32_t bitPos)
{
  size_t byteIndex = bitPos / 8;
  if (byteIndex >= mask.size()) {
    return;
  ...
  uint8_t bit = static_cast<uint8_t>(7 - (bitPos % 8));
  mask[byteIndex] = static_cast<uint8_t>(mask[byteIndex] & ~(1U << bit));
}

static void
trimTrailingZeros(std::vector<uint8_t>& mask)
{
...
Forwarder::onIncomingInterest(const Interest& interest, const FaceEndpoint& ingress)
{
  ...
  if (this->processBierControlInterest(interest, ingress)) {
    return;
  }
  ...
  if (this->processBierInterest(interest, ingress)) {
    return;
  }
...
std::vector<uint8_t>
Forwarder::findBierPrefixMask(const Name& name) const
{
  auto longest = m_config.bierPrefixMap.end();
  for (auto it = m_config.bierPrefixMap.begin(); it != m_config.bierPrefixMap.end(); ++it) {
    if (!it->first.isPrefixOf(name)) {
      continue;
    }
    if (longest == m_config.bierPrefixMap.end() || it->first.size() > longest->first.size()) {
      longest = it;
    }
  }

  if (longest == m_config.bierPrefixMap.end()) {
    return {};
  }
...
bool
Forwarder::processBierInterest(const Interest& interest, const FaceEndpoint& ingress)
{
  auto bierTag = interest.getTag<lp::BierBitStringTag>();
  std::vector<uint8_t> bierMask = bierTag != nullptr ? *bierTag : this->findBierPrefixMask(interest.getName());
  if (bierMask.empty()) {
    return false;
  }

  bool shouldDeliverLocally = false;
  if (m_config.bierLocalBit.has_value() && isBitSet(bierMask, *m_config.bierLocalBit)) {
    shouldDeliverLocally = true;
    clearBit(bierMask, *m_config.bierLocalBit);
  }

  std::map<Face*, std::vector<uint8_t>> egressMasks;
  for (const auto& [bitPos, faceId] : m_config.bierBitFaceMap) {
    if (!isBitSet(bierMask, bitPos)) {
      continue;
    }

    Face* egress = m_faceTable.get(faceId);
    if (egress == nullptr || egress->getId() == ingress.face.getId()) {
      continue;
    }
    setBit(egressMasks[egress], bitPos);
  }

  for (auto& [egress, mask] : egressMasks) {
    trimTrailingZeros(mask);
    if (mask.empty()) {
      continue;
    }

    Interest cloned(interest);
    cloned.setTag(make_shared<lp::BierBitStringTag>(std::move(mask)));
    NFD_LOG_DEBUG("processBierInterest out=" << egress->getId() << " interest="
                  << cloned.getName() << " nonce=" << cloned.getNonce());
    egress->sendInterest(cloned);
    ++m_counters.nOutInterests;
  }

  if (shouldDeliverLocally) {
    const_cast<Interest&>(interest).removeTag<lp::BierBitStringTag>();
    return false;
  }

  return !egressMasks.empty();
}

bool
Forwarder::processBierControlInterest(const Interest& interest, const FaceEndpoint&)
{
  static const Name bierCtlPrefix("/localhost/nfd/bier");
  const Name& name = interest.getName();
  if (!bierCtlPrefix.isPrefixOf(name)) {
    return false;
  }

  auto parseUint = [] (const name::Component& c) -> uint64_t {
    try {
      return std::stoull(c.toUri());
    }
    catch (const std::exception&) {
      NDN_THROW(ConfigFile::Error("Invalid BIER control argument '" + c.toUri() + "'"));
    }
  };

  if (name.size() == bierCtlPrefix.size() + 1 && name.at(-1).toUri() == "clear") {
    m_config.bierPrefixMap.clear();
    m_config.bierBitFaceMap.clear();
    m_config.bierLocalBit.reset();
    NFD_LOG_INFO("Cleared BIER runtime configuration");
    return true;
  }

  if (name.size() >= bierCtlPrefix.size() + 2 && name.at(bierCtlPrefix.size()).toUri() == "local-bit") {
    m_config.bierLocalBit = static_cast<uint32_t>(parseUint(name.at(bierCtlPrefix.size() + 1)));
    NFD_LOG_INFO("Set BIER local bit to " << *m_config.bierLocalBit);
    return true;
  }

  if (name.size() >= bierCtlPrefix.size() + 3 && name.at(bierCtlPrefix.size()).toUri() == "bit-face") {
    uint32_t bitPos = static_cast<uint32_t>(parseUint(name.at(bierCtlPrefix.size() + 1)));
    uint64_t faceId = parseUint(name.at(bierCtlPrefix.size() + 2));
    m_config.bierBitFaceMap.insert_or_assign(bitPos, faceId);
    NFD_LOG_INFO("Set BIER bit-face mapping bit=" << bitPos << " face=" << faceId);
    return true;
  }

  if (name.size() >= bierCtlPrefix.size() + 4 && name.at(bierCtlPrefix.size()).toUri() == "prefix") {
    std::vector<uint8_t> mask = decodeHexMask(name.at(bierCtlPrefix.size() + 1).toUri());
    if (mask.empty()) {
      NFD_LOG_WARN("Ignoring empty BIER prefix mask");
      return true;
    }
    Name prefix;
    for (size_t i = bierCtlPrefix.size() + 2; i < name.size(); ++i) {
      prefix.append(name.at(i));
    }
    if (prefix.empty()) {
      NFD_LOG_WARN("Ignoring empty BIER prefix mapping");
      return true;
    }
    m_config.bierPrefixMap.insert_or_assign(prefix, std::move(mask));
    NFD_LOG_INFO("Set BIER prefix mapping for " << prefix);
    return true;
  }

  NFD_LOG_WARN("Ignoring malformed BIER control Interest " << name);
  return true;
}
...
Forwarder::processConfig(const ConfigSection& configSection, bool isDryRun, const std::string&)
{
  ...
  for (const auto& pair : configSection) {
    ...
    else if (key == "bier_local_bit") {
      config.bierLocalBit = ConfigFile::parseNumber<uint32_t>(pair.second, pair.first, CFG_FORWARDER);
    }
    else if (key == "bier_prefix_map") {
      for (const auto& item : pair.second) {
        Name prefix(item.first);
        auto mask = decodeHexMask(item.second.get_value<std::string>());
        if (mask.empty()) {
          NDN_THROW(ConfigFile::Error("Empty bier mask for prefix '" + prefix.toUri() + "'"));
        }
        config.bierPrefixMap.insert_or_assign(std::move(prefix), std::move(mask));
      }
    }
    else if (key == "bier_bit_face_map") {
      for (const auto& item : pair.second) {
        uint32_t bitPos = 0;
        try {
          bitPos = static_cast<uint32_t>(std::stoul(item.first));
        }
        catch (const std::exception&) {
          NDN_THROW(ConfigFile::Error("Invalid bit position '" + item.first +
                                      "' in section '" + CFG_FORWARDER + ".bier_bit_face_map'"));
        }
        uint64_t faceId = ConfigFile::parseNumber<uint64_t>(item.second, item.first, "bier_bit_face_map");
        config.bierBitFaceMap.insert_or_assign(bitPos, faceId);
      }
    }
    else {
