...
GenericLinkService::encodeLpFields(const ndn::PacketBase& netPkt, lp::Packet& lpPacket)
{
  ...
  auto bierBitStringTag = netPkt.getTag<lp::BierBitStringTag>();
  if (bierBitStringTag != nullptr) {
    lpPacket.add<lp::BierBitStringField>(*bierBitStringTag);
  }
                                   ...
                                   const EndpointId& endpointId)
{
  ...
  if (firstPkt.has<lp::BierBitStringField>()) {
    interest->setTag(make_shared<lp::BierBitStringTag>(firstPkt.get<lp::BierBitStringField>()));
  }
