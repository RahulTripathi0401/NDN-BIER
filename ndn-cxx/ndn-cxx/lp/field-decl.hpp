...
template<typename TlvType>
struct DecodeHelper<TlvType, std::vector<uint8_t>>
{
...
template<typename encoding::Tag TAG, typename TlvType>
struct EncodeHelper<TAG, TlvType, std::vector<uint8_t>>
{
  static size_t
  encode(EncodingImpl<TAG>& encoder, const std::vector<uint8_t>& value)
  {
    if (value.empty()) {
      NDN_THROW(ndn::tlv::Error("NDNLP field of TLV-TYPE " + to_string(TlvType::value) +
                                " cannot be empty"));
    }
    return prependBinaryBlock(encoder, TlvType::value, {value.data(), value.size()});
  }
};
