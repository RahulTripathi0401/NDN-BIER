...
BOOST_AUTO_TEST_CASE(InterestTagRoundtrip)
{
  const std::vector<uint8_t> expectedMask = {0x80, 0x01};
  auto interest = makeInterest("/Hello/Bier", false, 50_ms);
  ...
  auto sentTag = face.sentInterests[0].getTag<lp::BierBitStringTag>();
  BOOST_REQUIRE(sentTag != nullptr);
  const auto& sentMask = sentTag->get();
  BOOST_CHECK_EQUAL_COLLECTIONS(sentMask.begin(), sentMask.end(), expectedMask.begin(), expectedMask.end());
}
