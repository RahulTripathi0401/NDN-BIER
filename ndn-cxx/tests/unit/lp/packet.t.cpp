...
BOOST_AUTO_TEST_CASE(EncodeDecodeBierBitString)
{
  static const std::vector<uint8_t> expectedMask = {0x80, 0x40, 0x01};
  Packet packet;
  packet.set<BierBitStringField>(expectedMask);
  Block wire = packet.wireEncode();

  Packet decoded;
  decoded.wireDecode(wire);
  BOOST_CHECK(decoded.has<BierBitStringField>());
  const auto decodedMask = decoded.get<BierBitStringField>();
  BOOST_CHECK_EQUAL_COLLECTIONS(expectedMask.begin(), expectedMask.end(),
                                decodedMask.begin(), decodedMask.end());
}
